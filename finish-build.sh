                                            ...
                                            if [ -d "$NEWROOT/home/abdallah" ]; then
                                                ...
                                                # BUILD a working Firefox profile (instead of deleting it!)
...
INSTALLS
                                            ...
                                            # Run the exact dracut command that fixes the freeze
                                            dracut -N --force --nofscks --nomdadm --add 'dmsquash-live' --add-drivers 'squashfs loop overlay' --omit 'nfs' --include /tmp/dracut-hooks/99-fix-sysroot.sh /lib/dracut/hooks/pre-pivot/99-fix-sysroot.sh "$WORK/boot/initrd.img"
                                            ...
                                            menuentry "EtherealOS - Verbose Debug Mode (VBOX SAFE)" {
